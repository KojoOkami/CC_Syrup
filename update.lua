...
local usage = [[
 update type
 Everything inside the directory will be 
 downloaded to downloads/CC_Syrup/.
  ]]
 
local title = "Updating "..gBranch
...
function getGithubContents( path )
    ...
    local response = http.get("https://api.github.com/repos/KojoOkami/CC_Syrup/contents//?ref="..gBranch)
    if response then
...
function downloadManager( path )
    ...
    for i,data in pairs(fType) do
        if data == "file" then
            checkPath = http.get("https://raw.github.com/KojoOkami/CC_Syrup/"..gBranch.."/"..fPath[i])
            if checkPath == nil then
                
                fPath[i] = fPath[i].."/"..fName[i]
            end
            local path = "/CC_Syrup/"..fPath[i]
            if not fileList.files[path] and not isBlackListed(fPath[i]) then
                fileList.files[path] = {"https://raw.github.com/KojoOkami/CC_Syrup/"..gBranch.."/"..fPath[i],fName[i]}
            end
        end
    end
    for i, data in pairs(fType) do
        if data == "dir" then
            ...
            if not fileList.dirs[path] then 
                ...
                fileList.dirs[path] = {"https://raw.github.com/KojoOkami/CC_Syrup/"..gBranch.."/"..fPath[i],fName[i]}
